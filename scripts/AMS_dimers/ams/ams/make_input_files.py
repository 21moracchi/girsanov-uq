...
for i in range(n_ams):
    os.system('mkdir ams_' + str(i))
    os.system('cp job* ams_' + str(i))
    os.system('cp run_ams.py  ams_' + str(i) + '/')
    
    f = open("run_ams.py", "rt")
    f_run = open("ams_" + str(i) + "/run_ams.py", "wt")
    rendered = expand_common_blocks(f.read())
    for line in rendered.splitlines(keepends=True):
        if "ams_seed" in line:
            line = line.replace("ams_seed = None", "ams_seed = " + str(ams_seeds[i]))
        if "dyn_seed" in line:
